# runs on dolfinx/dolfinx:nightly
...
def define_interior_eq(mesh, degree, submesh, submesh_to_mesh, value, traps=True):
    ...
    if traps:
        ...
        element = basix.ufl.mixed_element([element_CG])
    ...
    c_m = us[0]
    v_m = vs[0]
    F = ufl.inner(ufl.grad(c_m), ufl.grad(v_m)) * dx_r - val * v_m * dx_r
    if traps:
        k = 2
        p = 0.1
        n = 0.5
...
u_1, v_1s, F_11, m_to_t = define_interior_eq(mesh, 1, submesh_t, submesh_t_to_mesh, 0.0, traps=False)
u_0.name = "u_b"
u_1.name = "u_t"
